  ...
  setup() {
    ...
    const userAvatarUrl = computed(() => {
      return (
        ...
        "https://ui-avatars.com/api/?name=" + encodeURIComponent(user.value?.name || "User")
      )
    })
    ...
    const viewMode = computed({
      ...
      set: (value) => store.commit("SET_VIEW_MODE", value),
    })
    const sortBy = computed({
      get: () => store.state.sortBy,
      set: (value) => store.commit("SET_SORT_BY", value),
    })
    const sortOrder = computed({
      get: () => store.state.sortOrder,
      set: (value) => store.commit("SET_SORT_ORDER", value),
    })
    ...
    const categoryLabel = computed(() => {
      ...
      for (const topCategory of categories.value) {
        if (topCategory.children) {
          const subCategory = topCategory.children.find((c) => c.id === currentCategory.value)
    ...
    const openNewFolderModal = () => {
      window.dispatchEvent(new CustomEvent("open-modal", { detail: "new-folder-modal" }))
    }

    const openUploadModal = () => {
      window.dispatchEvent(new CustomEvent("open-modal", { detail: "upload-modal" }))
    }
    ...
    const openChangePasswordModal = () => {
      userDropdownOpen.value = false
      window.dispatchEvent(new CustomEvent("open-modal", { detail: "change-password-modal" }))
    }
    ...
    watch(
      ...
      (routeName) => {
        if (routeName === "recent" || routeName === "shared" || routeName === "favorites") {
